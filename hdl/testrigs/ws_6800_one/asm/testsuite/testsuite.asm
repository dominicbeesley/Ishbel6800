...
;       NAM BIGTST00
;       TTL **** 6800 TOTAL INSTRUCTION TEST ****
;       OPT LLEN=80
...
;      REQUIRES RAM AT LOCATIONS        'DATA' .. 'DATA'+0x200	*
;      REQUIRES ROM OR RAM AT LOCATIONS 'CODE' .. 'CODE'+0x2FFF	*

;      STARTAT LOCATION 'CODE'
...
;               LOAD ACCUMULATOR B (LDA)
		LDAB	DATA     ; DATA=0252,N=1,Z=0,V=0
A10:		BPL	A10
A11:		BEQ	A11
A12:		BVS	A12
		EORB	DATA
...
A14:		BMI	A14
		EORB	DATA+1
A15:		BNE	A15
		LDAB	DATA+3   ; DATA+3=00,J=0,Z=1,V=0
A16:		BNE	A16
;*******************************************

;               STORE ACCUMULATOR A (STA)
		LDAA	DATA     ; DATA=0252,N=1,Z=0,V=0
		STAA	TMP
A20:		BPL	A20
A21:		BEQ	A21
		BVS	$
...
A41:		BEQ	A41
		...
		EORA	TMP
A43:		BNE	A43
		LDAA	DATA+1		; DATA+1=0125
		EORA	TMP+1
A44:		BNE	A44
		LDS	DATA+1		; DATA+1=0125,DATA+2=0252,N=0
A45:		BMI	A45
		STS	TMP
		LDAA	DATA+1
		EORA	TMP
...
;               LOAD AND STORE INDEX REGISTORS (LDX&STX)
		LDX	DATA		; DATA=0252,DATA+1=0125,N=1,Z
A50:		BPL	A50
A51:		BEQ	A51
A52:		BVS	A52
		STX	TMP
...
A53:		BNE	A53
		...
		EORA	TMP+1
A54:		BNE	A54
		LDX	DATA+1		; DATA+1=0125,DATA+2=0252
A55:		BMI	A55
		STX	TMP
		LDAA	DATA+1
		EORA	TMP
...
;               LOGICAL OR ACCUMULATOR A (ORA)
		LDAA	DATA			; DATA=0252
		ORAA	DATA+1			; DATA+1=0125,N=1,Z=0,V=0
A801:		BPL	A801
A802:		BEQ	A802
A803:		BVS	A803
		EORA	DATA+5			; DATA+5=0377
A804:		BNE	A804
		LDAA	DATA+1
		ORAA	DATA
A805:		BPL	A805
...
;               ADDA CCUMULATOR A TO MEMORY (ADD)
		LDAA	DATA+1			; DATA+1=0125
...
A109:		BNE	A109
		LDAA	DATA+9			; DATA+9=0201(-127)
		ADDA	DATA+10			; DATA+10=0277(-65),N=0,Z=0
A110:		BMI	A110
A111:		BEQ	A111
A112:		BVC	A112
A113:		BCC	A113
		EORA	DATA+6			; DATA+6=0100
A114:		BNE	A114
		LDAA	DATA+9			; DATA+9=0201
		ADDA	DATA+5			; DATA+5=0377,N=1,Z=0,V=0,C=1
A115:		BPL	A115
A116:		BEQ	A116
A117:		BVS	A117
A118:		BCC	A118
		EORA	DATA+8			; DATA+8=0200
A119:		BNE	A119
		LDAA	DATA+5			; DATA+5=0377
...
;               ADDA CCUMULATOR B TO MEMORY
		LDAB	DATA+1			; DATA+1=0125
...
A207:		BVC	A207
A208:		BCC	A208
		EORB	DATA+7			; DATA+7=0124
A209:		BNE	A209
		LDAB	DATA+9			; DATA+9=0201(-127)
		ADDB	DATA+10			; DATA+10=0277(-65),N=0,Z=0
A210:		BMI	A210
A211:		BEQ	A211
A212:		BVC	A212
A213:		BCC	A213
		EORB	DATA+6			; DATA+6=0100
A214:		BNE	A214
		LDAB	DATA+9			; DATA+9"=""0201
		ADDB	DATA+5			; DATA+5=0377,N=1,Z=0,V=0,C=1
A215:		BPL	A215
A216:		BEQ	A216
A217:		BVS	A217
A218:		BCC	A218
		EORB	DATA+8			; DATA+8=0200
A219:		BNE	A219
		LDAB	DATA+5			; DATA+5=0377
...
A227:		BVS	A227
A228:		BCC	A228
		EORB	DATA+11			; DATA+11=0376
A229:		BNE	A229
		LDAB	DATA+3			; DATA+3=00
...
;               ADD WITH CARRY,ACCUMULATOR A (ADC)
		LDAA	DATA+1   		; DATA+1=0125
		ADDA	DATA+3			; DATA+3=0,RESULT=0125,
                				; CARRY SET TO ZERO
...
;               ADD WITH CARRY,ACCUMULATOR B (ADC)
		LDAB	DATA+1			; DATA+1=0125
		ADDB	DATA+3			; DATA+3=0,RESULT=0125,
						; CARRY SET TO ZERO
...
;      * * * * * * * * * * * * * * * * * * * * * * * * * * *
		...
		BRA	GRP6
;		PAGE
;       TTL **** 6800 TOTAL INSTRUCTION TEST - GROUP 6 ****
...
;           GROUP 6 TEST COMPLETE-RETURN TO START
		...
		BRA	GRP2
;		PAGE
;       TTL **** 6800 TOTAL INSTRUCTION TEST - GROUP 2 ****
...
;        ANDA CUMULATOR ADDRESSING
		LDX	#TMP 			; SETUP INDEX REG
		LDAA	#0xFF 			; PUT -1 IN TMP
		STAA	TMP
...
ZERO:		.byte 0x00 ;* 00   		; WAS OCTAL @00
OC001:		.byte 0x01 ;* 0001
OC003:		.byte 0x03 ;* 0003
OC007:		.byte 0x07 ;* 0007
OC017:		.byte 0x0F ;* 0017
OC025:		.byte 0x15 ;* 0025
OC037:		.byte 0x1F ;* 0037
OC052:		.byte 0x2A ;* 0052
OC053:		.byte 0x2B ;* 0053
OC077:		.byte 0x3F ;* 0077
OC122:		.byte 0x52 ;* 0122
OC124:		.byte 0x54 ;* 0124
OC125:		.byte 0x55 ;* 0125
OC126:		.byte 0x56 ;* 0126
OC127:		.byte 0x57 ;* * 0127
OC135:		.byte 0x5D ;* 0135
OC142:		.byte 0x62 ;* 0142
OC164:		.byte 0x74 ;* 0164
OC177:		.byte 0x7F ;* 0177
OC200:		.byte 0x80 ;* 0200
OC212:		.byte 0x8A ;* 0212
OC225:		.byte 0x95 ;* 0225
OC251:		.byte 0xA9 ;* 0251
OC252:		.byte 0xAA ;* 0252
OC254:		.byte 0xAC ;* 0254
OC272:		.byte 0xBA ;* 0272
OC300:		.byte 0xC0 ;* 0300
OC305:		.byte 0xC5 ;* 0305
OC320:		.byte 0xD0 ;* 0320
OC325:		.byte 0xD5 ;* 0325
OC350:		.byte 0xE8 ;* 0350
OC352:		.byte 0xEA ;* 0352
OC376:		.byte 0xFE ;* 0376
OC377:		.byte 0xFF ;* 0377
;		PAGE
;       TTL **** 6800 TOTAL INSTRUCTION TEST - GROUP 4 ****
...
;      TEST DAA
		...
		BRA	GRP5
;		PAGE
;       TTL **** 6800 TOTAL INSTRUCTION TEST - GROUP 5 ****
